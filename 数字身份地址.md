...
import com.kingdee.kchain.fabric.common.util.SecurityUtils;
...
public class DigitalIdentityUtil {
    ...
    public static String getAddress(String cert) {
        X509Certificate x509Certificate = SecurityUtils.bytesToCertificate(cert.getBytes());
...
1.Java中byte的大小为8bits，其范围是-128~127的。
2.java的二进制采用的是补码形式
...
2. Go计算的x、y的字节数组，与Java计算出来的首尾会有多一个“0”的情况。

- 经过多次推导测试，发现，需要将x、y以及sha256后的数组将前导零去掉。